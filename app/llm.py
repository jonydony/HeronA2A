...
def diagnose() -> dict:
    """TEMP deploy diagnostic: one raw call, status/location/body (secrets masked)."""
    if not configured():
    ...
    try:
        ...
        out["post_location"] = (p.headers.get("location") or "")[:200]
        out["post_body"] = p.text[:300]
    except Exception as e:
        out["post_error"] = str(e)[:300]
    return out
